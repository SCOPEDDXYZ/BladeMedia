...
print_banner() {
  printf '\n\033[1;36m'
  printf '=====================================\n'
  printf '   MediaBlade Organizer Installer    \n'
  printf '=====================================\n'
  printf '\033[0m'
}
...
main() {
  ...
  # Auto-detect MediaBlade root (finds docker-compose.yml).
  # Back-compat: also detects legacy /srv/blademedia installs.
  local mediablade_root=""
  if [[ -f docker-compose.yml ]]; then
    mediablade_root="$(pwd)"
  elif [[ -f /srv/mediablade/docker-compose.yml ]]; then
    mediablade_root="/srv/mediablade"
  elif [[ -f /srv/blademedia/docker-compose.yml ]]; then
    mediablade_root="/srv/blademedia"
  else
    mediablade_root="$(
      find /srv /opt /home \
        -name "docker-compose.yml" \
        \( -path "*/mediablade*" -o -path "*/blademedia*" \) \
        | head -1 \
        | xargs -I {} dirname {}
    )"
  fi

  if [[ -z "${mediablade_root}" ]]; then
    printf '\033[1;31mError: No MediaBlade docker-compose.yml found\033[0m\n' >&2
    printf 'Run from your stack root (contains docker-compose.yml) or check /srv/mediablade/\n' >&2
    exit 1
  fi

  printf '\033[1;32m✅ Found MediaBlade at: %s\033[0m\n' "${mediablade_root}"
  ...
  cd "${mediablade_root}" || exit 1

  # Install Python dependencies
  printf '\n\033[1;34m📦 Installing Python dependencies...\033[0m\n'
  sudo apt update || true
  sudo apt install -y python3 python3-requests python3-pip || \
    sudo yum install -y python3 python3-pip || \
    sudo dnf install -y python3 python3-pip

  # Best-effort fallback (for distros without python3-requests)
  python3 -c 'import requests' >/dev/null 2>&1 || (pip3 install --user requests || sudo pip3 install requests)
...
#!/usr/bin/env python3
"""
MediaBlade Organizer v2.1 - Unattended Service Edition
...
class MediaOrganizer:
    ...
    def send_discord(self, title: str, message: str, color: str = "success"):
        ...
        embed = {
            ...
            "footer": {"text": "MediaBlade Organizer v2.1"},
        }
        try:
    ...
    def organize_movie(self, video_path: Path) -> Optional[str]:
        ...
        if not self.dry_run:
            ...
            for sub in subs:
                if sub.exists():
                    shutil.move(str(sub), str(new_folder / sub.name))
    ...
    def run(self):
        ...
        msg = (
            f"**Movies**: {self.summary['movies_organized']} org, {self.summary['movies_skipped']} skip\n"
...
def main():
    parser = argparse.ArgumentParser(description="MediaBlade Organizer")
...
EOF
  ...
  sudo tee /etc/systemd/system/mediablade-organizer.service > /dev/null << EOF
[Unit]
...
[Service]
Type=oneshot
WorkingDirectory=${mediablade_root}
ExecStart=/usr/bin/python3 ${mediablade_root}/media_organizer.py . ${org_args}
User=$(id -u)
Group=$(id -g)
StandardOutput=append:${mediablade_root}/organizer.log
StandardError=append:${mediablade_root}/organizer.log
TimeoutStartSec=300
...
EOF
  ...
  sudo tee /etc/systemd/system/mediablade-organizer.timer > /dev/null << 'EOF'
[Unit]
...
EOF
  ...
  sudo systemctl enable mediablade-organizer.timer
  sudo systemctl start mediablade-organizer.timer
  ...
  sudo systemctl status mediablade-organizer.timer --no-pager

  printf '\n📋 Logs: tail -f %s/organizer.log\n' "${mediablade_root}"
  printf '🧪 Manual test: python3 media_organizer.py . --dry-run\n'
  printf '⏹️  Stop: sudo systemctl stop mediablade-organizer.timer\n'
}
