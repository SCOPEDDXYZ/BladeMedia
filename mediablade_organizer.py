...
class MediaOrganizer:
    ...
    def organize_movie(self, video_path: Path) -> Optional[str]:
        ...
        logger.info("📽️  [%s] [%s] <- %s", title, year, video_path.name)
        if not self.dry_run:
    ...
    def organize_tv(self, video_path: Path) -> Optional[str]:
        ...
        subs = self.find_associated_subs(video_path)
        sub_dests = [
            new_season_folder / f"{new_filename.rsplit('.', 1)[0]}{sub.suffix}"
            for sub in subs
        ]

        logger.info("📺 [%s] S%02dE%02d <- %s", show, season, episode, video_path.name)
        if not self.dry_run:
    ...
    def run_movies(self):
        """Organize movies."""
        logger.info("🎬 Scanning movies: %s", self.movies_root)
    ...
    def run_tv(self):
        """Organize TV shows."""
        logger.info("📺 Scanning TV: %s", self.tv_root)
    ...
    def run(self):
        ...
        if total_organized > 0:
            ...
            logger.info("✅ Organized %d files total", total_organized)
        else:
...
def main():
    ...
    parser.add_argument("root_path", help="MediaBlade root (e.g. /srv/mediablade)")
